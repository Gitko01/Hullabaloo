...
public class MobAttractorBlockEntity extends BlockEntity implements ExtendedScreenHandlerFactory {
    ...
    private final PropertyDelegate energyAmountPropertyDelegate = new PropertyDelegate() {
        ...
        public int get(int index) {
            if (index == 0) {
                ...
                // energy consumption
                return (int) (range.getX() * range.getY() * range.getZ());
            } else {
                return getCooldown();
            }
        }
    ...
    public static void tick(World world, BlockPos pos, BlockState state, MobAttractorBlockEntity be) {
        ...
        if (be.attractorCooldown >= SPEED && world.isReceivingRedstonePower(pos) && be.energyStorage.getAmount() >= be.range.getX() * be.range.getY() * be.range.getZ()) {
            be.attractorCooldown = 0;
            be.energyStorage.amount -= be.range.getX() * be.range.getY() * be.range.getZ();
